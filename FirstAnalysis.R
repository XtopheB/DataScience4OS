...
# =============================================================================
# A Step-by-Step Analysis - Descriptive Statistics
# NSB Course - Paro
...
# 
# # --- Plotting ---
# library(ggplot2)      # Grammar of graphics (also loaded via tidyverse)
# library(RColorBrewer) # Color palettes for plots
# library(GGally)       # Extensions to ggplot2: pairs plots, etc.
# 
# # --- Tables and result presentation ---
# library(knitr)        # Dynamic report generation utilities
# library(modelsummary) # Publication-ready model and data summary tables
# library(sjPlot)       # Regression tables and model visualizations
# library(gt)           # Grammar of tables: highly customizable HTML tables
# library(gtsummary)    # Clinical-style summary tables
...
# Any code using random number generation (e.g. sample(), rnorm()) will produce
# the same results each time the script is run.
set.seed(2512)

# Set a clean, minimal ggplot2 theme globally.
# theme_set() applies this theme to every subsequent plot automatically.
theme_set(theme_minimal() + theme(panel.grid = element_blank()))

# Define custom color palettes used consistently throughout the analysis.
# Using named vectors lets us map colors to factor levels explicitly.

# Main brand color (used for neutral highlights)
SIAP.color <- "#0385a8"

# Colors for the 'sex' variable
sex_colors <- c(
  "Men"   = "coral",
  "Women" = "#9B59B6"
)

# Colors for the 'age_group' variable
age_colors <- c(
  "Young"  = "#F39C12",
  "Middle" = "#C0504D",
  "Old"    = "#8da0cb"
...
# How to use these palettes in ggplot2:
#   For points and lines: scale_color_manual(values = sex_colors)
#   For bar / area fills: scale_fill_manual(values = sex_colors)
...
data <- data %>%
  ...
  mutate(
    # Create an age group variable from the continuous age variable.
    # 'case_when' is a readable, vectorised if-else chain.
    age_group = case_when(
      age < 20              ~ "Young",
...
pred_data <- expand.grid(
  education = seq(min(data$education), max(data$education), length.out = 100),
  sex       = unique(data$sex),
  age       = mean(data$age)    # Hold age constant at its mean
)
